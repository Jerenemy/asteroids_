...
#WHAT TO DO: 
#FIX FUCKING LEVEL COUNTER: broken on restart now because i set it back to zero when sship lives go to zero(which is also wrong cause not general for when lives decrease) 
#FUCK THIS
#UNDO ALL THE THINGS I DID WITH LEVEL COUNTER BECAUSE I DON'T WANT IT TO RESTART THE LEVEL WHEN SSHIP DIES
#UNDO THAT, THEN FIX BIP COUNTER TO ALSO NOT STOP IF SSHIP DESTROYED
#THEN CAN ACTUALLY DO STUFF
#change bip times to be based on iterations, not ticks
#add PHASES, difficulty increases up to a point, then resets and increases again. Progressively stays harder for longer
#PHASES will make the game 10x more satisfying to play, DO THIS ASAP!!!
#add a phase when many auto spaceships come in all at once with no asteroids
...
from Classes.Spaceship import SpaceshipLives
from Classes.Spaceship import SpaceshipAutomated
from Classes.Spaceship import SpaceshipAutomatedList

from Classes.Asteroid import Asteroid
from Classes.AsteroidList import AsteroidList
